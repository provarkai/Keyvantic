...
| **Proposal Generator** | Documents, Master Library content (Phase 0), ideally Semantic Search (Phase 2) | Assembles proposals by pulling approved content blocks from the library (e.g. via semantic search over `05 Sales`/`08 Products`); benefits from Phase 2's retrieval quality but can ship a manual-select v1 earlier. |
