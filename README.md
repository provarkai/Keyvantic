...
- **Proposal Generator**: drafts a client proposal section-by-section from a template, using that client's own approved Discovery Notes plus firm-wide Methodology/Framework docs — retrieval is hard-isolated per client, and commercial-terms sections are never AI-drafted. See [`docs/10-proposal-generator-spec.md`](./docs/10-proposal-generator-spec.md).
- **Branding**: light/dark theme, responsive layout, Keyvantic navy/gold visual identity.

See [`docs/07-development-roadmap.md`](./docs/07-development-roadmap.md) for what's intentionally deferred (native diagram/flowchart tooling, upload-based video hosting, embedded PDF viewer, deeper knowledge-graph analytics) and the phased plan for future modules (CRM, Project Management, AI Transformation Assessment, Client Portal, LMS, Research Publishing, Executive/Financial Dashboards, Internal AI Copilot).
